...
template<typename GV, int dimG>
void GeneralSurfaceExtractor<GV, dimG>::update(const ElementDescriptor<GV>& descr)
{
      ...
      if (descr.contains(elit))
      {
        ...
        for (IsIter is = this->_gv.ibegin(*elit); is != this->_gv.iend(*elit); ++is)
        {
          ...
          if (is->boundary())
          {
            ...
            const int face_corners = Dune::ReferenceElements<ctype, dim>::general(gt).size(num_in_parent, 1, dim);
            ...
            if (!added)
            {
              ...
              this->_elmtInfo[eindex] = new ElementInfo(simplex_index, elit, face_corners - 2);
              added = true;
            }
            else
            {
              // the element info exists already, register the additional face(s)
            ...
            case 4 :
              ...
              {
                // get the number of the vertex in the parent element
                vertex_numbers[i] = orientedSubface<dim>(gt, num_in_parent, i);
              ...
              temp_faces.push_back(FaceInfo(simplex_index++, eindex, num_in_parent, 2));
...
public:
  ...
  {
    STDOUTLN("This is GeneralSurfaceExtractor on a <" << GV::dimension << "," << GV::dimensionworld << "> grid working in " << Base::dimw << " space expecting faces of type " << GeometryType(GeometryType::cube, Base::dim) << "!");
  }
};
